    ...
    topStories.then(feed => {
        ...
        Promise.all(feed.stories).then(function(stories){ 
            ...
            for (let i in stories) {
                ...
                html += '<li class="list__item story" itemscope itemtype="http://schema.org/Article"><h2 class="story__title" itemprop="name"><a href="'+stories[i].url+'" target="_blank">'+stories[i].title+'</a></h2><div class="story__summary"><span itemprop="author">By: '+stories[i].by+'</span><span datetime="'+ date.getFullYear() + '-' + date.getMonth() + '-' + date.getDate() +'"  itemprop="datePublished">Posted: '+ date.getDate() + '-' + date.getMonth() + '-' + date.getFullYear() +'</span></div></li>';
            }
